...
if (sessionStorage.getItem("userArray") != null) {
  if (sessionStorage.getItem("userArray").length == 2) {
    users = [];
  } else {
...
if (sessionStorage.getItem("phoneArray") != null) {
  if (sessionStorage.getItem("phoneArray").length == 2) {
    phones.push(
...
$(document).ready(function() {
  ...
  $("#btnModalSignIn").click(() => {
    if (
      $("#signInUserName").val() == "sa" &&
      $("#signInPassword").val() == "sapassword"
    ) { 
  ...
  //Khi bấm nút đăng nhập với tư cách admin

  $("#btnModalSignInAdmin").click(() => {
    if (
      $("#signInUserName").val() == "sa" &&
      $("#signInPassword").val() == "sapassword"
    ) {
      ...
      alert("2")
      $("#resultSignIn").html("Tên tài khoản admin hoặc mật khẩu không đúng");
    }
  });
  ...
  $("#btnSignOut").click(() => {
	...
	$("#btnMuaNgay").click(function() {
		...
		var price = $("#priceAgain").text();
		var soLuong = $("#soLuong").val();
		$("#total").html(price * soLuong);
	})

  //Khi thay đổi số lượng mua
